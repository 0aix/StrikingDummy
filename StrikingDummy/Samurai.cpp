...
namespace StrikingDummy
{
	Samurai::Samurai(Stats& stats) :
		...
		shifu_base_gcd(lround(floor(0.1f * floor(0.87f * floor(this->stats.ss_multiplier * BASE_GCD))))),
		shifu_iai_gcd(lround(floor(0.1f * floor(0.87f * floor(this->stats.ss_multiplier * IAI_GCD))))),
		auto_gcd(lround(floor(0.1f * floor(1000.0f * this->stats.auto_delay)))),
		shifu_auto_gcd(lround(floor(0.1f * floor(0.87f * floor(1000.0f * this->stats.auto_delay)))))
	{
	...
	bool Samurai::can_use_action(int action) const
	{
		...
		case HIGANBANA_2:
			return gcd_timer.ready && tsubame_cd.ready && tsubame_sen == 1;
		case TENKA_2:
			return gcd_timer.ready && tsubame_cd.ready && tsubame_sen == 2;
		case MIDARE_2:
			return gcd_timer.ready && tsubame_cd.ready && tsubame_sen == 3;
		case POT:
	...
	float Samurai::get_damage(int action) const
	{
		...
		case MIDARE:
			...
			break;
		case HIGANBANA_2:
			potency = HIGANBANA_POTENCY * KAITEN_MULTIPLIER;
			break;
		case TENKA_2:
			potency = TENKA_POTENCY * KAITEN_MULTIPLIER;
			break;
		case MIDARE_2:
			potency = MIDARE_POTENCY * KAITEN_MULTIPLIER;
			break;
		}
		// floor(ptc * wd * ap * det * traits) * chr | * dhr | * rand(.95, 1.05) | ...
		return potency * stats.potency_multiplier * ((weaponskill && kaiten.count > 0) ? KAITEN_MULTIPLIER : 1.0f) * (jinpu.count > 0 ? JINPU_MULTIPLIER : 1.0f) * (pot.count > 0 ? stats.pot_multiplier : 1.0f) * stats.expected_multiplier;
	}
	...
	void Samurai::get_state(float* state)
	{
		...
		state[13] = pot.count > 0;
		state[14] = pot.time / (float)POT_DURATION;
		...
		state[34] = senei_cd.ready;
		state[35] = senei_cd.time / (float)SENEI_CD;
		state[36] = ikishoten_cd.ready;
		state[37] = ikishoten_cd.time / (float)IKISHOTEN_CD;
		state[38] = tsubame_cd.ready;
		state[39] = tsubame_cd.time / (float)TSUBAME_CD;
		state[40] = pot_cd.ready;
		state[41] = pot_cd.time / (float)POT_CD;
		state[42] = gcd_timer.ready;
		state[43] = gcd_timer.time / 250.0f;
	}
